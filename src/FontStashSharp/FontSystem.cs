	...
	unsafe class FontSystem : IDisposable
	{
		...
		public double DrawText(SpriteBatch batch, double x, double y, string str, Color color, float depth)
		{
			...
			for (int i = 0; i < str.Length; i += char.IsSurrogatePair(str, i) ? 2 : 1)
			{
				...
				if (!glyph.IsEmpty)
				{
					...
					var destRect = new RectangleF((float)(x + q.X0),
												(float)(y + q.Y0),
												(float)(q.X1 - q.X0),
												(float)(q.Y1 - q.Y0));
					...
					var scale = destRect.Height / sourceRect.Height;
		...
		public float DrawText(SpriteBatch batch, float x, float y, string str, Color[] glyphColors, float depth)
		{
			...
			for (int i = 0; i < str.Length; i += char.IsSurrogatePair(str, i) ? 2 : 1)
			{
				...
				if (!glyph.IsEmpty)
				{
					...
					var pos2 = new Vector2((float)(x + q.X0), (float)(y +q.Y0));
					var scale = destRect.Height / sourceRect.Height;
		...
		public float DrawText(SpriteBatch batch, float x, float y, StringBuilder str, Color color, float depth)
		{
			...
			for (int i = 0; i < str.Length; i += StringBuilderIsSurrogatePair(str, i) ? 2 : 1)
			{
				...
				if (!glyph.IsEmpty)
				{
					...
					var pos = new Vector2((float)(x + q.X0), (float)(y + q.Y0));
					var rotation = 0.0f;
		...
		public float DrawText(SpriteBatch batch, float x, float y, StringBuilder str, Color[] glyphColors, float depth)
		{
			...
			for (int i = 0; i < str.Length; i += StringBuilderIsSurrogatePair(str, i) ? 2 : 1)
			{
				...
				if (!glyph.IsEmpty)
				{
					...
					var posa = new Vector2((float)(x + q.X0), (float)(y + q.Y0));
					var scale = destRect.Height / sourceRect.Height;
		...
		private void GetQuad(FontGlyph glyph, FontGlyph prevGlyph, float spacing, ref double x, ref double y, ref FontGlyphSquad q)
		{
			if (prevGlyph != null)
			{
				...
				x += prevGlyph.XAdvance * prevGlyph.Font.Scale + adv * prevGlyph.Font.Scale;
			}
			...
			//x += glyph.XAdvance * glyph.Font.Scale;
		}
	}
}
